...
/// Bounds the raw bytes that the SSE codec may retain for one incomplete event.
/// Parsing, UTF-8 validation, comments, and multi-line `data:` semantics remain
/// owned by `eventsource-stream`.
    ...
    pub(super) fn observe(&mut self, bytes: &[u8]) -> Result<()> {
        for &byte in bytes {
            ...
            self.bytes_in_event = self
                .bytes_in_event
                .checked_add(1)
                .ok_or_else(|| anyhow::anyhow!("OpenAI SSE event byte count overflowed"))?;
            if self.bytes_in_event > self.max_event_bytes {
                bail!(
                    "OpenAI SSE event exceeded the {} byte limit",
...
mod tests {
    ...
    #[test]
    fn guard_resets_for_lf_crlf_and_cr_event_boundaries() {
    ...
    #[tokio::test]
    async fn mature_codec_preserves_utf8_across_transport_partitions() {
        ...
        assert_eq!(event.data, "{\"delta\":\"中文🙂\"}");
    }

    #[tokio::test]
    async fn mature_codec_supports_comments_and_multiline_data() {
    ...
    #[tokio::test]
    async fn mature_codec_rejects_invalid_utf8_instead_of_replacing_it() {
