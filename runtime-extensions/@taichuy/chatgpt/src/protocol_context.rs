...
pub(crate) enum ChatGptWireProtocol {
    Chat,
    Responses,
}

impl ChatGptWireProtocol {
...
pub(crate) fn restore_protocol_context(
    ...
    for (name, value) in &envelope.body {
        ...
        if protocol == ChatGptWireProtocol::Responses && name == "responses_optional_tools" {
...
mod tests {
    ...
    fn same_protocol_accept_language_is_preserved_for_chat_and_responses() {
        for (protocol, source_protocol) in [
            (ChatGptWireProtocol::Chat, "openai_chat"),
            (ChatGptWireProtocol::Responses, "openai_responses"),
        ] {
            let envelope: ProtocolContextEnvelope = serde_json::from_value(json!({
    ...
    fn wp_d2c_current_abi_rejects_typed_body_collisions_for_chat_and_responses() {
        for (protocol, source_protocol, collision) in [
            (ChatGptWireProtocol::Chat, "openai_chat", "messages"),
            (
                ChatGptWireProtocol::Responses,
                "openai_responses",
                "reasoning",
    ...
    fn wp_r3_foreign_context_is_not_injected_while_same_protocol_fields_stay_fail_closed() {
        ...
        let (body, restored) =
            restore_protocol_context(ChatGptWireProtocol::Responses, json!({}), Some(&foreign))
