...
fn count_tokens_result(input: Value) -> Value {
    ...
    let wire_body = serde_json::from_value::<ProviderInvocationInput>(generate_input)
        .ok()
        .and_then(|input| build_typed_chat_completion_body(&input).ok())
        .unwrap_or_else(|| count_tokens_fallback_body(&input));
    count_tokens::provider_estimate(wire_body, &input)
}
...
fn build_http_client(config: &ProviderConfig) -> Result<reqwest::Client> {
    thread_local! {
        static RECENT: std::cell::RefCell<Option<(ProviderConfig, reqwest::Client)>> =
    ...
    RECENT.with(|recent| {
        ...
        let client = build_http_client_uncached(config)?;
        *recent.borrow_mut() = Some((config.clone(), client.clone()));
        Ok(client)
    })
}

fn build_http_client_uncached(config: &ProviderConfig) -> Result<reqwest::Client> {
