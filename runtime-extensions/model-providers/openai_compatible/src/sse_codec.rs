...
impl SseEventSizeGuard {
    ...
    pub(super) fn observe(&mut self, bytes: &[u8]) -> Result<()> {
        for &byte in bytes {
            ...
            self.bytes_in_event = self.bytes_in_event.checked_add(1).ok_or_else(|| {
                anyhow::anyhow!("OpenAI-compatible SSE event byte count overflowed")
            })?;
            if self.bytes_in_event > self.max_event_bytes {
                bail!(
                    "OpenAI-compatible SSE event exceeded the {} byte limit",
...
mod tests {
    ...
    async fn codec_supports_comments_and_multiline_data() {
        ...
        let events = source.eventsource().collect::<Vec<_>>().await;
        assert_eq!(events.len(), 1);
        assert_eq!(events[0].as_ref().unwrap().data, "{\"choices\":\n[]}");
    }
