...
mod tests {
    ...
    #[tokio::test]
    async fn different_transport_partitions_decode_to_identical_text() {
        let wire = b"data: {\"type\":\"content_block_delta\",\"index\":0,\"delta\":{\"type\":\"text_delta\",\"text\":\"same\"}}\n\n";
        let whole = stream::iter(vec![Ok::<_, std::io::Error>(wire.to_vec())])
            .eventsource()
            .collect::<Vec<_>>()
            .await;
        let partitioned = stream::iter(vec![
            Ok::<_, std::io::Error>(wire[..3].to_vec()),
            Ok(wire[3..17].to_vec()),
            Ok(wire[17..].to_vec()),
        ])
        .eventsource()
        ...
        let whole: serde_json::Value =
            serde_json::from_str(&whole[0].as_ref().unwrap().data).unwrap();
        let partitioned: serde_json::Value =
            serde_json::from_str(&partitioned[0].as_ref().unwrap().data).unwrap();
        assert_eq!(whole["delta"]["text"], "same");
        assert_eq!(whole["delta"]["text"], partitioned["delta"]["text"]);
    }
