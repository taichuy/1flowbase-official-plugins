...
test('provider workflows call the current host plugin CLI path', () => {
  ...
  assert.match(releaseWorkflow, /node host\/scripts\/node\/plugin\/cli\.js "\$\{package_args\[@\]\}"/);
...
test('provider-release supports manual repair dispatches', () => {
  ...
  assert.match(workflow, /if \[ "\$\{provider_code\}" = "all" \]; then/);
  assert.match(
    workflow,
...
test('provider-release extracts package metadata from plugin CLI output instead of assuming JSON stdout', () => {
  ...
  assert.match(
    workflow,
    /package_output="\$\(node host\/scripts\/node\/plugin\/cli\.js "\$\{package_args\[@\]\}"\)"/
  );
  assert.match(
    workflow,
...
test('provider-release removes same-platform release assets before uploading replacements', () => {
  ...
  assert.match(
    workflow,
    /node scripts\/provider-release\/assets\.mjs cleanup-lines --base "\$\{ASSET_BASENAME\}"/
  );
